...
export default class MainScene extends Phaser.Scene{
    ...
    preload()
    {
        this.load.image('backgroud','./images/map.png');
    ...
    create()
    {
        ...
        this.cars=[];
        this.cars.push(this.add.existing(new Car(this, 10*128-64,2*128+64,'yellow.car',false)));
        this.cars.push(this.add.existing(new Car(this, 128-64,4*128+64,'yellow.car',true)));
    ...
    update(time)
    {
        ...
        if(Phaser.Geom.Intersects.RectangleToRectangle(playerRect,motorcycleRect)){
            console.log('vrrrrrrrrruuuuum...');
            this.scene.restart();
        }

        for(let i =0; i< this.cars.length;i++){
            if(this.cars[i].overlaps(this.player)){
                console.log('passadinho a ferro');
                this.scene.restart();
            }
        }
    }
